...
export async function preloadSound() {
  ...
  for (const note of notes) {
    ...
    if (!Object.values(preloadCache).flatMap(tone => tone.note).includes(note)) {
      let pitch = 0
      let noteSound = note
      ...
      if (Object.values(preloadCache).flatMap(tone => tone.note).includes(noteSound)) {
        const player = preloadCache.filter(e => e.note == noteSound)[0].player
